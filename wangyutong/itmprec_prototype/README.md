...
步骤一：创建 conda 环境。执行 conda create -n itmprec python=3.10，然后 conda activate itmprec。

步骤二：安装依赖包。执行 pip install openai numpy -i https://mirrors.aliyun.com/pypi/simple/

可选依赖（如需本地 token 计数或工具函数）：pip install tiktoken langchain-core -i https://mirrors.aliyun.com/pypi/simple/

步骤三：验证环境。执行 python -c "import openai; print(openai.__version__)" 和 python -c "import numpy; print(numpy.__version__)" 确认安装成功。

步骤四：配置 API Key（仅 API 模式需要）。Windows CMD 执行 set DEEPSEEK_API_KEY=你的_api_key，Linux 或 Mac 或 Git Bash 执行 export DEEPSEEK_API_KEY=你的_api_key。
